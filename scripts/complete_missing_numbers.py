...
python scripts/complete_missing_numbers.py annotations fixed_annotations F_38_1.TextGrid
"""
...
def complete_missing_numbers_for_single_annotation(text_folder, output_folder, text_file):
...
def complete_missing_numbers(text_folder, output_folder):
    ...
    for text_file in sorted(os.listdir(text_folder)):
        complete_missing_numbers_for_single_annotation(text_folder, output_folder, text_file)


if __name__ == "__main__":
    print(sys.argv)
    if len(sys.argv) == 4:
        complete_missing_numbers_for_single_annotation(sys.argv[1], sys.argv[2], sys.argv[3])
    if len(sys.argv) != 3:
